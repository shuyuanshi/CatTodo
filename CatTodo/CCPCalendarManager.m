...
- (void)calendar {
    if (!av) {
        ...
        av.frame = CGRectMake(0, 90, main_width, main_height - 90 - 49);
        av.manager = self;
...
- (NSString *)startTitle {
    if (!_startTitle) {
        _startTitle = [@"Pick" stringByAppendingFormat:@"\n%@",@"Date"];
    }
    return _startTitle;
}
...
//单选有过去
+ (void)show_signal_past:(UIViewController *)vc complete:(completeBlock)complete {
