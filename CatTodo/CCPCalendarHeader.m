...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"manager.selectArr"]) {
        ...
        if (arr.count == 0) {
            ...
            self.manager.startTitle = [NSString stringWithFormat:@"%ld/%02ld\n%@",[date getMonth],[date getDay],[date weekString]];
            ...
            self.manager.endTitle = [NSString stringWithFormat:@"%ld/%02ld\n%@",[date2 getMonth],[date2 getDay],[date2 weekString]];
        }
        [self displayLabel];
    }
    else if ([keyPath isEqualToString:@"manager.startTitle"]) {
        if (![self.manager.startTitle isEqualToString:[@"Pick" stringByAppendingFormat:@"\n%@",@"Date"]]) {
            r_btn.hidden = YES;
        }
        else {
            r_btn.hidden = YES;
